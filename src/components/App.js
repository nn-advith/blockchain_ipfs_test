...
//import ABI for contract. Can be generated by compiling contract using Remix. The remaining abi part is generated after compiling contract.
...
class App extends Component {
  ...
  //load and get files on component mount
  ...
  //for navigation. no react-router :(
  changePage = page => {
    this.setState({page: page, vOp: '', cFilename:''});
  }


  //get hash value from Verify page. In retrospect, could have been made into a local state.
  getHash = e => {
    e.preventDefault()
  ...
  uploadFile = description => {
        ...
        ipfs.add(this.state.buffer, (err, result) => {
          ...
          //call uploadFile method of the DStorage contract. The input values are verified. On successful completion, loading stopped and window reloaded to 
          //get the new files. Alternatively, loadBlockChainData() can be called but this fucntion should be converted into an asynchronous function.
  ...
  //commented out because of issue wth verifying, Blob type and arraybuffer type clash. Fix later

  // //download the certificate generated with file name as file<random 8 digit number>.pdf
  // downloadFile = () => {
 
  //   //options for file ouput of html2pdf
  //   var options = {
  //     margin: 1,
  //     filename: this.state.cFilename
  //   }
  ...
  generateFileName = () => {
    this.setState({cFilename: 'file'+Math.random().toString().slice(2,10)})
  }


  //generate pdf certificate and upload it directly to IPFS and hash.
  createAndUpload = description => {
    ...
    var element = document.getElementById('certificate');
    html2pdf().set(options).from(element).toPdf().save().output('blob').then((data) => { //outputPdf() to prevent autodownload
      try {

        //blob data type is of form ArrayBuffer. 
        //Should be converted to uint8Array which is done using the FileReader and then using Buffer.from(Filereader result)
    ...
    return (
        ...
        { this.state.loading
          ...
          <Create 
            ...
            generateFileName={this.generateFileName}
          />//create pdf file component
          :
          //verify
          <Verify 
